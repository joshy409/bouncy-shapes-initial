...
EllipseRenderer::EllipseRenderer(std::shared_ptr<BrushDeviceResource> brush, Color4F color, float radius)
	: _brushPtr(brush), _color(color), _radius(radius)
{}

void EllipseRenderer::Render(ID2D1HwndRenderTarget & target, const MPoint2F & position) const
{
	...
	auto ellipse = D2D1::Ellipse(center, _radius, _radius*7);
